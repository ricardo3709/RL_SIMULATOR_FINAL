...
def run_policy_theta(initial_theta, sim_duration, warm_up_duration, policy, fixed_normalizer):
    test_env = ManhattanTrafficEnv()
...
if __name__ == "__main__":
    ...
    fixed_theta_result = run_fixed_theta(initial_theta, sim_duration, warm_up_duration)
    policy_theta_result = run_policy_theta(initial_theta, sim_duration, warm_up_duration, policy, fixed_normalizer)
    results = [fixed_theta_result, policy_theta_result]
    # 使用pickle保存结果
    with open('16h_simulation_results.pkl', 'wb') as f:
