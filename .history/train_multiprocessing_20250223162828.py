...
def main():
    ...
    parser.add_argument("--eval_freq", default=2, type=int)       # How often (epochs) we evaluate
    parser.add_argument("--save_freq", default=2, type=int)       # How often (epochs) we save
    parser.add_argument("--data_collection_step", default=48*8, type=int)   # how many steps to collect data each run 48*8 (8hours)
    ...
    parser.add_argument("--load_model", default="TD3_AUTOENCODER_28") # Model load file name, "" doesn't load, "default" uses file_name TD3_AUTOENCODER_
    parser.add_argument("--num_processes", default=8, type=int)     # Number of processes to use 
    parser.add_argument("--training_repeat", default=1, type=int)   # Number of times to train the policy
    parser.add_argument("--cycles", default=100, type=int)           # Number of cycles to run the data_collection and training
    ...
    if args.load_model != "":
        ...
        actor_lr = 1e-5
        critic_lr = 3e-5
        policy.actor_optimizer = torch.optim.Adam(policy.actor.parameters(), lr=actor_lr)
        policy.critic_optimizer = torch.optim.Adam(policy.critic.parameters(), lr=critic_lr)
    ...
    # 为每个进程创建独立的 Queue
    process_queues = [manager.Queue(maxsize = args.data_collection_step) for _ in range(args.num_processes)]

    # create pickle file for each env
    print("Initializing Simulator Environments...")
    for i in range(args.num_processes):
        print(f"Creating env {i}")
        env = ManhattanTrafficEnv()
        ...
        with open(save_env_name, 'wb') as f:
            pickle.dump(env, f)
        print(f"Env {i} created and saved")
    print("All simulator enviroments have been initialized")
    ...
    for cycle in range(args.cycles):
        ...
        print("Start to run data collection in parrallel")
        for i in range(args.num_processes):
        ...
        if False: # TEMP: buffer collecting -> True, Training -> False
            if replay_buffer.size > 0:
                if cycle > 0:
                    ...
                    if HALF_REQ:
                        ...
                        if MAP_NAME == 'Utrecht':
                            np.savez('mean_std_data_Utrecht_FULL.npz', mean=mean, std=std)
