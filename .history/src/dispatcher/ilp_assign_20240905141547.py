...
def ilp_assignment(veh_trip_pairs: List[Tuple[Veh, List[Req], List[Tuple[int, int, int, float]], float, float]], #list[[Veh, list[Req], list[(int, int, int, float)], float, float]]
                #    considered_rids: List[int],
                   ...
                   config: ConfigManager, reward_theta) -> List[int]:
    ...
    # Objective: minimize the sum of anticipatory cost
    object_score = 0.0

    # for RL: return sum of cost
    anticipatory_cost_sum = 0.0
    user_cost_sum = 0.0
    operator_cost_sum = 0.0
    user_costs = []
    operator_costs = []
    costs = []

    for i in range(len(veh_trip_pairs)):
        tiny_weight = 1e-3 * (i + 1)
        # Anticipatory Method Objective 
        # cost = anticipatory_cost(veh_trip_pairs[i][2], veh_trip_pairs[i][0]) 
        # costs.append(cost)
        operator_cost, user_cost = anticipatory_cost(veh_trip_pairs[i][2], veh_trip_pairs[i][0])
        operator_costs.append(operator_cost)
        user_costs.append(user_cost)
        ...
        anticipate_method_object_score = operator_cost + user_cost - reward_theta * reward
        object_score += veh_trip_pairs_check[i] * (anticipate_method_object_score + tiny_weight)
    ...
    # Calculate the sum of anticipatory cost
    for i in range(len(veh_trip_pairs)):
        # anticipatory_cost_sum += veh_trip_pairs_check[i].getAttr(GRB.Attr.X) * costs[i]
        operator_cost_sum += veh_trip_pairs_check[i].getAttr(GRB.Attr.X) * operator_costs[i]
        user_cost_sum += veh_trip_pairs_check[i].getAttr(GRB.Attr.X) * user_costs[i]
    ...
    return selected_veh_trip_pair_indices, operator_cost_sum, user_cost_sum #index of selected vehicle-trip pairs, optimized by ILP
    # return selected_veh_trip_pair_indices, anticipatory_cost_sum #index of selected vehicle-trip pairs, optimized by ILP
...
def anticipatory_cost(new_schedule: list, veh: Veh):
    ...
    if len(new_schedule) == 2: # only newly inserted req
        users_cost = first_term
        operator_cost = PO * get_timeCost(new_schedule[0][0], new_schedule[1][0])
        # anticipatory_cost_value = first_term + PO * get_timeCost(new_schedule[0][0], new_schedule[1][0])
        return operator_cost, users_cost
    ...
    users_cost = first_term + second_term
    operator_cost = third_term
