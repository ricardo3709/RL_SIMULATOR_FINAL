...
from src.simulator.route_functions import map_node_to_area
from src.dispatcher.ilp_assign import reward_function
...
def apply_anticipation_to_schedule(schedule: list):
    last_node = schedule[-1][0]
    last_zone = map_node_to_area(last_node)
    # the idea is to put theta in the routing part as well
    # but I am not sure how to do it
    # since only if you instert the new request at last node, you can get the reward
    # the solver would just put the new request at the last node
    # So theta is not useful in the routing partq
