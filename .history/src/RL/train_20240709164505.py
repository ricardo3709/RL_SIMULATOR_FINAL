...
#   ddpg_agent = ddpg_agent.to(device)
    ...
    with open('training_log_1.txt', 'w') as log_file:
        for epoch in range(epochs):
            ...
            while not done:
                environment.simulator.current_cycle_rej_rate = [] # reset the rejection rate to record the new cycle
                for step in tqdm(range(int(RL_STEP_LENGTH)), desc=f'Current State'): # 2.5mins, 10 steps
                    environment.simulator.system_time += TIME_STEP
                    environment.simulator.run_cycle() # Run one cycle(15s)
                ...
                # # Copy current env to get temp env
                # temp_env = copy.deepcopy(environment)
                # # update temp env one step forward to get next_state
                # for step in tqdm(range(int(RL_STEP_LENGTH)), desc=f'Next State'): 
                #     temp_env.simulator.system_time += TIME_STEP
                #     temp_env.simulator.run_cycle() # Run one cycle(15s)

                # if len(temp_env.past_rejections) < 3:
                #     current_past_rej = 0.0
                #     last_past_rej = 0.0
                # else:
                #     current_past_rej = temp_env.past_rejections[-2]
                #     last_past_rej = temp_env.past_rejections[-3]

                # if len(temp_env.past_thetas) < 3:
                #     current_past_theta = 5.0
                #     last_past_theta = 5.0
                # else:
                #     current_past_theta = temp_env.past_thetas[-2]
                #     last_past_theta = temp_env.past_thetas[-3]

                # next_state = temp_env.simulator.get_simulator_state_by_areas()
                # next_x = next_state.clone().detach().requires_grad_(True)
                # next_x = next_x.to(device)
                # current_past_rej_tensor = torch.tensor([current_past_rej], dtype=torch.float32).to(device).unsqueeze(0).expand(next_x.size(0), -1)
                # current_past_theta_tensor = torch.tensor([current_past_theta], dtype=torch.float32).to(device).unsqueeze(0).expand(next_x.size(0), -1)
                # next_x = torch.cat((next_x, current_past_rej_tensor, current_past_theta_tensor), dim=1)
                # next_graph_data = Data(x=next_x, edge_index=edge_index)
                # next_state_encoded = gnn_encoder(next_graph_data) 

                
                critic_loss, actor_loss = ddpg_agent.update_policy(last_reward, current_state_encoded, next_state_encoded)
