...
class DDPG_Agent(nn.Module):
    ...
    def update_policy(self, last_reward, current_state, last_state):
        ...
        with torch.no_grad():
            ...
            target_Q= current_reward + self.discount * self.critic_target(next_state, next_action)

        with torch.autograd.set_detect_anomaly(False):
            # calculate critic loss and backward 
            current_action = self.actor(current_state)
            ...
            actor_loss = -torch.mean(self.critic(current_state, self.actor(current_state)).mean()
