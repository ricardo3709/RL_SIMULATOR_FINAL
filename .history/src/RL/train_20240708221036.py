    ...
    with open('training_log_1.txt', 'w') as log_file:
        for epoch in range(epochs):
            ...
            while not done:
                ...
                current_past_rej_tensor = torch.tensor([current_past_rej], dtype=torch.float).to(device).global_info.unsqueeze(0).expand(num_nodes, -1)
                current_x = torch.cat(current_x, torch.tensor([current_past_rej, current_past_theta], dtype=torch.float).to(device))
                ...
                current_state_encoded = gnn_encoder(current_graph_data, current_past_rej, current_past_theta)  # encode the state (1,32)
                last_state_encoded = gnn_encoder(last_graph_data, last_past_rej, last_past_theta)  # encode the state (1,32)
                ...
                last_action = environment.past_actions[-2] if len(environment.past_actions) > 1 else [[0.0001]]
                critic_loss, actor_loss = ddpg_agent.update_policy(last_action, reward, current_state_encoded, last_state_encoded)
