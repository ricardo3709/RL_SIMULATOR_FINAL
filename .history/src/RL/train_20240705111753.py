import torch
from RL.models_ori import GNN_Encoder, Actor, Critic
    ...
    with open('training_log_1.txt', 'w') as log_file:
        for epoch in range(epochs):
            ...
            while not done:
                ...
                current_state_encoded = gnn_encoder(current_graph_data, last_rej_rate)  # encode the state (1,32)
                last_state_encoded = gnn_encoder(last_graph_data, last_rej_rate)  # encode the state (1,32)
                ...
                last_action = environment.past_actions[-2]
