...
class GNN_Encoder(nn.Module):
    def __init__(self, num_features=NUM_FEATURES, hidden_dim=64, output_dim=32):
        ...
        self.normalizer = Normalizer(size=num_features+2)
...
class Normalizer:
    def __init__(self, size, eps=1e-2, device='cpu'):
        self.size = size
        self.eps = eps
        self.device = device
        self.n = torch.zeros((1, size), device=device)
        self.mean = torch.zeros(size, device=device)
        self.mean_diff = torch.zeros(size, device=device)
        self.var = torch.zeros(size, device=device)

    def observe(self, x):
        x = x.view(1, -1)  # Ensure x is of shape (1, size)
        self.n += 1  # Update the count
        last_mean = self.mean.clone()
