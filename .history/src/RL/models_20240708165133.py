...
class Actor(nn.Module):
    ...
    def forward(self, actor_x):
        ...
        assert -2 < self.layer3(actor_x) < 2 # check if the input is within range of tanh
...
class OUNoise:
    ...
    def noise(self):
        ...
        self.state = x + dx
        return torch.clamp(torch.tensor(self.state * self.scale).float(), min=-0.22, max=0.02)
