...
class GNN_Encoder(nn.Module):
    def __init__(self, num_features=NUM_FEATURES, hidden_dim=64, output_dim=32):
        ...
        self.normalizer = Normalizer(size=num_features)

    def forward(self, data, last_rej_rate, last_theta):
...
class DDPG_Agent(nn.Module):
    def __init__(self, state_dim, action_dim, max_action):
        ...
        self.actor = Actor(state_dim + 2, action_dim, max_action)  # state_dim + 1 to account for rejection rate
        self.critic = Critic(state_dim + 2, action_dim) # state_dim is the output dimension from GNN without rejection rate
        self.normalizer = Normalizer(size=state_dim + 2)
        ...
        # Target networks
        self.actor_target = Actor(state_dim + 2, action_dim, max_action)
        self.critic_target = Critic(state_dim + 2, action_dim)
...
class Actor(nn.Module):
    ...
    def forward(self, actor_x):
        ...
        actor
        # Normalize input features
        for i in range(actor_x.size(0)):
            self.normalizer.observe(actor_x[i, :])
        actor_x = torch.stack([self.normalizer.normalize(actor_x[i, :]) for i in range(actor_x.size(0))])
...
class Critic(nn.Module):
    ...
    def forward(self, critic_x, u):
        ...
        critic_x_normalized = critic_x.detach().clone().requires_grad_()
        # Normalize input features
        for i in range(critic_x_normalized.size(0)):
            self.normalizer.observe(critic_x_normalized[i, :])
        critic_x = torch.stack([self.normalizer.normalize(critic_x_normalized[i, :]) for i in range(critic_x_normalized.size(0))])
