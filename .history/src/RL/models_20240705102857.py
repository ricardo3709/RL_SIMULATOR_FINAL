...
class DDPG_Agent(nn.Module):
    ...
    def update_policy(self, action, reward, cat_state, edge_index, last_rej_rate):
        ...
        if not isinstance(cat_state, torch.Tensor):
            cat_state = torch.FloatTensor(cat_state)
        cat_state = cat_state.to(device)
        

        graph_cat_state = Data(x=cat_state, edge_index=edge_index)
        cat_state_encoded = self.gnn_encoder(cat_state,last_rej_rate)

        # Calculate target Q
        with torch.no_grad():
            target_action = self.actor_target(cat_state_encoded)
            target_Q = self.critic_target(cat_state_encoded, target_action)
            target_Q = reward + (self.discount * target_Q)

        with torch.autograd.set_detect_anomaly(True):

            # 接着计算 Critic 的损失并进行反向传播
            current_Q = self.critic(cat_state_encoded, action)
            ...
            actor_loss = -self.critic(cat_state_encoded.detach(), self.actor(cat_state_encoded.detach())) 
