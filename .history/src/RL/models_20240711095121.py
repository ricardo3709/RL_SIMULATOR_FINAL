...
class DDPG_Agent(nn.Module):
    ...
    def select_action(self, state):
        ...
        noise = torch.random.normal(-abs(action)/10, abs(action)/10, action.size())  # Add noise to the action
    ...
    def update_policy(self, last_reward, current_state, last_state):
        ...
        with torch.autograd.set_detect_anomaly(False):
            ...
            # after_update = [param for param in self.actor.parameters()]

            # for before, after in zip(before_update, after_update):
            #     print(torch.equal(before, after))  # Should print False if parameters are being updated
...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, max_action):
        ...
        self.layer4 = nn.Linear(state_dim+NUM_FEATURES, action_dim)
        self.max_action = max_action
        self.apply(self._init_weights)
        self.normalizer = Normalizer(size=state_dim+NUM_FEATURES)
    ...
    def forward(self, actor_x):
        ...
        # Update normalizer with the current batch
        self.normalizer.update(actor_x)

        # Normalize actor_x
        actor_x = self.normalizer.normalize(actor_x)

        actor_x = torch.tanh(self.ln4(self.layer4(actor_x))) * self.max_action
...
class Critic(nn.Module):
    ...
    def forward(self, critic_x, u):
        ...
        # print(f"Critic: {self.layer3.weight}")
        return critic_x

class Normalizer:
    def __init__(self, size, epsilon=1e-8, device=device):
