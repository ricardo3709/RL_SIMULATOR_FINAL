...
class GNN_Encoder(nn.Module):
    def __init__(self, num_features=NUM_FEATURES, hidden_dim=16, output_dim=64):
    ...
    def forward(self, state, edge_index, batch_size, rej_rate, theta_value):
        ...
        # # check h3 and see if saturated
        # print(f"max h3: {h3.max()}, min h3: {h3.min()}")
        # print(f"avg h3: {h3.mean()}, std h3: {h3.std()}")
        # Final projection with normalization
        x = self.fc(h3)
        # x = self.out_ln(x)
        x = x.view(batch_size, -1) #[b, 32*16]
        # check x and see if saturated
        # print(f"max x: {x.max()}, min x: {x.min()}")
        # print(f"avg x: {x.mean()}, std x: {x.std()}")
...
class Actor(nn.Module):
    ...
    def forward(self, state):
        ...
        # print(f"avg x1: {x.mean()}, std x1: {x.std()}")
        x = F.leaky_relu(self.l2(x))
        # print(f"avg x2: {x.mean()}, std x2: {x.std()}")
        x = F.leaky_relu(self.l3(x))
        # print(f"avg x3: {x.mean()}, std x3: {x.std()}")
        x = F.leaky_relu(self.l4(x))
        # print(f"avg x4: {x.mean()}, std x4: {x.std()}")
        x = self.l5(x)
        # print(f"avg x5: {x.mean()}, std x5: {x.std()}")
...
class Critic(nn.Module):
    ...
    def forward(self, state, action):
        ...
        # print(f"avg q1_l1: {q1.mean()}, std q1_l1: {q1.std()}")
        q1 = F.leaky_relu(self.l2(q1))
        # print(f"avg q1_l2: {q1.mean()}, std q1_l2: {q1.std()}")
        q1 = F.leaky_relu(self.l3(q1))
        # print(f"avg q1_l3: {q1.mean()}, std q1_l3: {q1.std()}")
        q1 = F.leaky_relu(self.l4(q1))
        # print(f"avg q1_l4: {q1.mean()}, std q1_l4: {q1.std()}")

        q1 = self.l5(q1)
        # print(f"avg q1_l5: {q1.mean()}, std q1_l5: {q1.std()}")
...
class TD3(object):
    def __init__(
        ...
        self.actor = Actor(state_dim=gnn_output_dim*NUM_AREA, action_dim=action_dim, max_action=max_action).to(device)
        self.actor_target = copy.deepcopy(self.actor)
        self.actor_optimizer = torch.optim.Adam(self.actor.parameters(), lr=actor_lr)

        # self.critic = Critic(state_dim=NUM_AREA*9, action_dim=1).to(device)
        self.critic = Critic(state_dim=gnn_output_dim*NUM_AREA, action_dim=action_dim).to(device)
    ...
    # self.critic_target = copy.deepcopy(self.critic)
        ...
        if MISSING_REWARD_PREDICTOR:
            ...
            self.gnn_encoder.load_state_dict(torch.load(filename + "_gnn_encoder", weights_only=True))
