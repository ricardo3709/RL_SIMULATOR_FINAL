...
class Normalizer:
    ...
    def observe(self, x):
        self.n.add_(1)  # 使用 in-place 操作符
        last_mean = self.mean.clone()
        self.mean.add_((x - self.mean) / self.n)  # 使用 in-place 操作符
        self.mean_diff.add_((x - last_mean) * (x - self.mean))  # 使用 in-place 操作符
        self.var.copy_((self.mean_diff / self.n).clamp(min=self.eps))  # 使用 in-place 操作符
