...
class Actor(nn.Module):
    ...
    def forward(self, actor_x):
        ...
        # Normalize weights
        mean = weights.mean(dim=2, keepdim=True)
        std = weights.std(dim=2, keepdim=True)
        weights = (weights - mean) / st
        # weights = F.softmax(weights, dim=2) #normalize weights, sum to 1

        weighted_sum = torch.sum(weights * actor_x, dim=2)

        final_action = torch.clamp(weighted_sum, -1, 1)
