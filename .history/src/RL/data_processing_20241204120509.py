...
def feature_preparation(state):
    ...
    rej_counts_areas_list = [sum(rej_counts_areas[i]) for i in range(NUM_AREA)]
    attraction_counts_areas_list = [sum(attraction_counts_areas[i]) for i in range(NUM_AREA)]
    ...
    gen_counts_areas_list = torch.tensor(gen_counts_areas_list,dtype=torch.float32, device=device)/req_super_div
    rej_counts_areas_list = torch.tensor(rej_counts_areas_list,dtype=torch.float32, device=device)/req_super_div
    attraction_counts_areas_list = torch.tensor(attraction_counts_areas_list,dtype=torch.float32, device=device)/req_super_div
    ...
    features_tensor = torch.stack(
        [avaliable_veh_areas, 
         ...
         gen_counts_areas_list, 
         rej_counts_areas_list, 
         attraction_counts_areas_list],
        dim=0
