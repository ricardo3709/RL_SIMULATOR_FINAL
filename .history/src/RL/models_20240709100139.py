...
class Actor(nn.Module):
    ...
    def forward(self, actor_x):
        ...
        actor_x = actor_x.unsqueeze(0).unsqueeze(0) #[1,1,63]
        weights = self.conv1d(actor_x)
        weights = F.softmax(weights, dim=2) #normalize weights to sum to 1

        weighted_sum = torch.sum(weights * actions, dim=2)
