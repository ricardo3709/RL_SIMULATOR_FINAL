...
class Simulator_Platform(object):
    ...
    def __init__(self, simulation_start_time_stamp: float, config: ConfigManager, init_type='RANDOM'):
        ...
        if MAP_NAME == 'Manhattan': # [ReqID Oid Did ReqTime Size]
            ...
            # read isolated nodes from pkl
            with open(PATH_ISOLATED_NODES, 'rb') as f:
                isolated_nodes = pickle.load(f)
                
            node_id_uniform = list(range(1,NUM_NODES+1,NUM_NODES//FLEET_SIZE[0]))
            for i in range(FLEET_SIZE[0]):
                if init_type == 'UNIFORM':
                    # Initialize the vehicles with uniform initial positions.
                    while node_id_uniform[i] in isolated_nodes:
                        node_id_uniform[i] += 1
                    self.vehs.append(Veh(i, self.system_time, node_id_uniform[i], VEH_CAPACITY[0]))
                elif init_type == 'RANDOM':
                    # Initialize the vehicles with random initial positions.
                    node_id_random = np.random.randint(1, NUM_NODES+1)
                    while node_id_random in isolated_nodes:
                        node_id_random = np.random.randint(1, NUM_NODES+1)
                    self.vehs.append(Veh(i, self.system_time, node_id_random, VEH_CAPACITY[0]))
    ...
    def get_current_cycle_request(self, current_time ) -> list:
        ...
        if MAP_NAME == 'Manhattan':
            if current_time % (3600*8) == 0:
                self.req_loader(current_time, last_req_ID)
        elif MAP_NAME == 'Utrecht':
            if current_time % (3600*3) == 0:
                self.req_loader(current_time, last_req_ID)
    ...
    #     if MAP_NAME == 'Utrecht':
    #         PATH_REQUESTS = f"{ROOT_PATH}/Utrecht/Utrecht_Requests.csv"
    #         # 指定数据类型和需要的列
    #         temp_req_matrix = pd.read_csv(
    #             PATH_REQUESTS,
    #             dtype={
    #                 'ReqID': np.int32,
    #                 'Oid': np.int32,
    ...
    #         # 使用vectorized操作代替循环
    #         reqs_data = temp_req_matrix.values
    #         self.reqs.extend(
    #             Req(int(row[0]), int(row[1]), int(row[2]), int(row[3]), int(row[4]))
    #             for row in reqs_data
    #         )

    #     elif MAP_NAME == 'Manhattan':
    #         PATH_REQUESTS = f"{ROOT_PATH}/NYC/Yellow_Taxi_Data/2016_03/"
    ...
    #         # 优化读取过程
    #         temp_req_matrix = pd.read_csv(
    #             os.path.join(PATH_REQUESTS, SELECTED_FILE),
    #             # dtype={
    #             #     'ReqID': np.int8,
    #             #     'ReqTime': np.int8,
    #             #     'Origin': np.int8,
    ...
    #         # 过滤size>3的请求，使用vectorized操作
    #         valid_reqs = temp_req_matrix[temp_req_matrix['Size'] <= 3].values
            
    #         # 批量创建Req对象
    #         self.reqs.extend(
    #             Req(int(row[0]), int(row[1]), int(row[2]), int(row[3]), int(row[4]))
    #             for row in valid_reqs
    #         )
    ...
    def req_loader(self, current_sim_time, last_req_ID, fixed_day=None):
        ...
        if MAP_NAME == 'Utrecht':
            PATH_REQUESTS = f"{ROOT_PATH}/Utrecht/Utrecht_Requests.csv"
            with open(PATH_REQUESTS, 'r') as f:
            ...
            if ENV_MODE == 'TEST':
                ...
                # SELECTED_FILE = FILE_NAME + str(int(day)) + '.csv' 
                SELECTED_FILE = FILE_NAME + str(10) + '.csv'
                ...
                if fixed_day:
                    ...
                    day = np.random.randint(1, 31)
                    # random_start_time = np.random.randint(0, 20*3600) # 0 to 20 hours
                    # day = 1
                    SELECTED_FILE = FILE_NAME + str(day) + '.csv'
            ...
            temp_req_matrix['ReqID'] += last_req_ID
            temp_req_matrix['ReqTime'] += current_sim_time
            # temp_req_matrix['ReqTime'] += random_start_time 
