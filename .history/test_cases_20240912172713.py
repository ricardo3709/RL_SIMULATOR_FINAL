...
if __name__ == "__main__":
    ...
    initial_thetas = [0, 30.0, 60.0]
    results = Parallel(n_jobs=6)(delayed(func)(theta, sim_duration, warm_up_duration) 
                                for theta in initial_thetas 
                                for func in [run_fixed_theta, run_policy_theta])
    #save results
    with open('fixed_policy_simulation_results.pkl', 'wb') as f:
        pickle.dump(results, f)
    ...
    # policy_decay_rewards = calculate_rewards(policy_rejection_rates, policy_users_costs)
    # fixed_decay_rewards = calculate_rewards(fixed_rejection_rates, fixed_users_costs)

    # #pack all results
    # results_0 = [fixed_rejection_rates, fixed_anticipatory_costs, fixed_operators_costs, fixed_users_costs, fixed_idle_veh_num_list, fixed_decay_rewards]
    # results_1 = [policy_rejection_rates, policy_anticipatory_costs, policy_operators_costs, policy_users_costs, policy_idle_veh_num_list, policy_decay_rewards, policy_theta_list[5:]]
    # results = [results_0, results_1]

    # results = prune_results(results, sim_duration, warm_up_duration, initial_theta)

    # plot_all_results_both(results, sim_duration, warm_up_duration, [], n)
