...
class Veh(object):
    ...
    def get_node_and_capacity_in_time(self, target_time):
        # no route
        accumulated_time = 0
        ...
        if len(self.route) == 0:
            ...
            for node in self.route:
                time_to_node = get_timeCost(self.current_node, node)
                accumulated_time += time_to_node
                if node == self.schedule[i][0]: #node is the ori/des of a request
                    if self.schedule[i][1] == 1: #PU node
                        current_load += self.schedule[i][2]
                    elif self.schedule[i][1] == -1: #DO node
                        current_load -= self.schedule[i][2]
                    else:
                        Exception("Invalid node type")
                    i += 1
                if accumulated_time >= target_time:
                    return node, self.capacity - current_load
            return self.route[-1], self.capacity - current_load
